...
def main():
    ...
    base_weight = [0.1 * np.random.randn(6, 25), 0.1 * np.random.randn(25, 3)]

    gaussian = [[np.random.randn(6, 25) * 0.1, np.random.randn(25, 3) * 0.1] for a in range (population_size)]
    ...
    for generation in range(generations):
        ...
        base_weight[0] = base_weight[0] + 0.01 * average_1
        base_weight[1] = base_weight[1] + 0.01 * average_2
        np.save('baseweight_0a.npy', base_weight[0])
        np.save('baseweight_1a.npy', base_weight[1])
       # print base_weight
