...
def main():
    ...
    base_weight = [0.1 * np.random.randn(2, 10), 0.1 * np.random.randn(10, 3)]

    gaussian = [[np.random.randn(2, 10) * 0.1, np.random.randn(10, 3) * 0.1] for a in range (population_size)]
    ...
    for generation in range(generations):
        ...
        np.save('baseweight_0m.npy', base_weight[0])
        np.save('baseweight_1m.npy', base_weight[1])
       # print base_weight
